  ...
  const addTodo = (text) => {
    if (text.trim() !== '') {
      const newTodo = {
        id: Date.now(),
        text: text.trim(),
        x: window.innerWidth / 2 - 130, // Center horizontally
        y: 0 // Will be positioned in the render method
      };
  ...
  const handleDragEnd = (event) => {
    ...
    setTodos(todos.map(todo => {
      if (todo.id === active.id) {
        ...
        // Ensure task stays within viewport
        const maxX = window.innerWidth - 300; // Assuming max width of 300px for task
